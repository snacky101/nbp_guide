...
>  Tip : SSH로 서버에 접근한 이후 다른 작업을 수행하다보면, Broken Pipe 라는 메세지와 함께 세션이 끊어질 때가 꽤 많은데 서버가 끊어지지 않도록 하기 위해서는 백그라운드에 데몬으로 실행시키면 됩니다. 확인은 해보지 않았으나 별 문제없이 적용될 것으로 보입니다. 적용 방식은 실행 커맨드 뒤에 `&`를 붙이시면 됩니다. ( 예. `node app.js &` ) 실행 시키신 후에, 엔터를 몇 번 더 쳐보시면 다시 커맨드를 수행할 수 있는 상태가 됩니다.

<br>

- 프로세스 종료하기

    일반적으로 그냥 `node app.js`와 같이 실행했을 경우에는 "Ctrl + C" 로 보통 빠져나오면 프로세스가 종료되지만 `node app.js &`와 같이 백그라운드에서 실행할 경우, 다시 노드를 실행시킬 경우 아래와 같은 에러 문구와 함께 프로세스가 충돌이 날 수 있습니다.
    ...
    이제 이 번호를 가진 프로세스를 아래의 커맨드를 통해 종료해 주시면 다시 문제없이 node를 실행시키실 수 있습니다.
